...
async function send(to: string, subject: string, html: string): Promise<void> {
  ...
  try {
    ...
    logger.info(`Email sent: ${subject} → ${to.split('@')[0]}@***`);
  } catch (err: any) {
...
export async function sendReviewPromptEmail(
  ...
  await send(
    ...
    wrapHtml('Rate Your Doctor', body),
  );
}
...
export async function sendAppointmentConfirmationEmail(
  ...
  patientFirstName: string,
  doctorName: string,
  appointmentDate: string,   // e.g. "25 March 2026"
  ...
  await send(
    patientEmail,
    `Appointment Confirmed — ${doctorName} on ${appointmentDate}`,
    wrapHtml('Appointment Confirmed', body),
  );
}
...
export async function sendDoctorVerificationEmail(
  ...
  await send(
    doctorEmail,
    subject,
    wrapHtml(action === 'approve' ? 'Profile Verified' : 'Verification Update', action === 'approve' ? approvedBody : rejectedBody),
  );
}
