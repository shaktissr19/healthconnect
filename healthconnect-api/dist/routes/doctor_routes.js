 ...
 */
router.get('/doctor/profile', auth_1.authenticate, (0, roleGuard_1.requireRole)('DOCTOR'), async (req, res) => {
    try {
 ...
 */
router.put('/doctor/profile', auth_1.authenticate, (0, roleGuard_1.requireRole)('DOCTOR'), async (req, res) => {
    try {
 ...
 */
router.put('/doctor/profile/availability', auth_1.authenticate, (0, roleGuard_1.requireRole)('DOCTOR'), async (req, res) => {
    try {
 ...
 */
router.put('/doctor/profile/consultation-modes', auth_1.authenticate, (0, roleGuard_1.requireRole)('DOCTOR'), async (req, res) => {
    try {
 ...
 */
router.get('/doctor/analytics', auth_1.authenticate, (0, roleGuard_1.requireRole)('DOCTOR'), async (req, res) => {
    try {
        const profile = await prisma.doctorProfile.findUnique({
            where: { userId: req.user.id },
 ...
 * Search HealthConnect patients by name or HC ID.
 * Returns ONLY: id, hcId, firstName, lastName, age, city — NO health data.
 * Requires: DOCTOR role
 */
router.get('/doctor/patients/search', auth_1.authenticate, (0, roleGuard_1.requireRole)('DOCTOR'), async (req, res) => {
    try {
        ...
        // Single char — ask for one more character
        if (q.length === 1)
            return res.status(400).json({ success: false, message: 'Search query must be at least 2 characters' });
        // Build where — empty q = browse all (50), typed q = filtered search (10)
        ...
        // Check existing consents for these patients
        ...
        // Check pending unread SYSTEM notifications this doctor sent
 ...
 * Sends an in-app SYSTEM notification to the patient.
 * No schema change — uses existing Notification.data (Json) to carry request payload.
 * Patient accepts via existing POST /patient/consents endpoint.
 * Requires: DOCTOR role
 */
router.post('/doctor/access-request', auth_1.authenticate, (0, roleGuard_1.requireRole)('DOCTOR'), async (req, res) => {
    try {
        const userId = req.user?.userId ?? req.user?.id;
        ...
        // Block if active consent already exists
        ...
        // Block if pending request notification already sent by this doctor
        ...
        await prisma.notification.create({
            data: {
                ...
                data: {
                    ...
                    doctorName,
                    doctorSpec: spec,
                    hcDoctorId: dp.hcDoctorId ?? '',
                    isVerified: dp.isVerified,
                    patientId,
                },
            },
        });
        // TODO: wire your email service here to send accept/reject email to patient
        // emailService.sendDoctorAccessRequestEmail({ patientUserId: patient.userId, doctorName, spec })
        return res.json({
            success: true,
            data: { message: 'Access request sent. Patient will be notified in-app and by email.', patientId, doctorId },
        });
    }
    catch (e) {
 ...
 * Returns all sent access requests for this doctor:
 *   - PENDING  → unread SYSTEM notifications with DOCTOR_ACCESS_REQUEST payload
 *   - ACCEPTED → PatientConsent records with status ACTIVE
 *   - REJECTED → PatientConsent records with status REVOKED
 * Requires: DOCTOR role
 */
router.get('/doctor/access-requests', auth_1.authenticate, (0, roleGuard_1.requireRole)('DOCTOR'), async (req, res) => {
    try {
        const userId = req.user?.userId ?? req.user?.id;
        const dp = await prisma.doctorProfile.findUnique({ where: { userId }, select: { id: true } });
        if (!dp)
        ...
        // 2. Pending notifications sent by this doctor (unread = patient hasn't acted yet)
        const pendingNotifs = await prisma.notification.findMany({
            where: {
                type: 'SYSTEM',
                isRead: false,
        ...
        const acceptedPatientIds = new Set(consents.map(c => c.patientId));
        // Enrich pending rows with patient name/hcId
        const pendingPatientIds = pendingNotifs
            .map(n => n.data?.patientId)
            .filter((id) => !!id && !acceptedPatientIds.has(id));
        ...
        const pendingRows = pendingNotifs
            ...
            return pid && !acceptedPatientIds.has(pid);
        })
            .map(n => {
            const pid = n.data?.patientId;
            ...
            return {
                ...
                patientHcId: p?.user.registrationId ?? '',
                status: 'PENDING',
...
//# sourceMappingURL=doctor_routes.js.map