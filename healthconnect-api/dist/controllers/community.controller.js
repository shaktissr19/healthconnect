...
exports.getPeopleLikeMe = exports.getCommunityHealthScore = exports.getMyBookmarks = exports.toggleBookmark = exports.votePoll = exports.createPoll = exports.getCommunityPolls = exports.adminReviewCommunityRequest = exports.adminListCommunityRequests = exports.getMyCommunityRequest = exports.submitCommunityRequest = exports.removeReaction = exports.reactToPost = exports.deleteComment = exports.createComment = exports.getPostComments = exports.deletePost = exports.updatePost = exports.createPost = exports.getRecentPosts = exports.getCommunityPosts = exports.followCommunity = exports.leaveCommunity = exports.joinCommunity = exports.getCommunity = exports.getRecommendedCommunities = exports.getFeaturedCommunities = exports.getCommunities = void 0;
...
const submitCommunityRequest = async (req, res, next) => {
    try {
        ...
        const userId = req.user?.userId ?? null; // optionalAuth — may be null for guests
        if (!communityName)
            return apiResponse_1.ApiResponse.error(res, 'BAD_REQUEST', 'Community name is required', 400);
        if (!reason?.trim())
            return apiResponse_1.ApiResponse.error(res, 'BAD_REQUEST', 'Reason is required', 400);
        ...
        if (userId) {
            ...
            if (existing) {
                return apiResponse_1.ApiResponse.success(res, existing, 'You already have a pending request');
            }
        }
        ...
        return apiResponse_1.ApiResponse.created(res, request, 'Request submitted. Our team will review within 48 hours.');
    }
    catch (e) {
        next(e);
    }
...
const getMyCommunityRequest = async (req, res, next) => {
    try {
        const userId = req.user?.userId;
        if (!userId)
            return apiResponse_1.ApiResponse.error(res, 'UNAUTHORIZED', 'Authentication required', 401);
        const request = await prisma.communityRequest.findFirst({
            where: { requestedBy: userId },
            orderBy: { createdAt: 'desc' },
        });
        if (!request)
            return apiResponse_1.ApiResponse.notFound(res, 'No request found');
        return apiResponse_1.ApiResponse.success(res, request);
    }
    catch (e) {
        next(e);
    }
};
exports.getMyCommunityRequest = getMyCommunityRequest;
// ─── GET /api/v1/communities/admin/requests ───────────────────────────────────
const adminListCommunityRequests = async (req, res, next) => {
    try {
        if (req.user?.role !== 'ADMIN')
            return apiResponse_1.ApiResponse.forbidden(res, 'Admin access required');
...
exports.adminListCommunityRequests = adminListCommunityRequests;
// ─── PATCH /api/v1/communities/admin/requests/:id ────────────────────────────
const adminReviewCommunityRequest = async (req, res, next) => {
    try {
        if (req.user?.role !== 'ADMIN')
            return apiResponse_1.ApiResponse.forbidden(res, 'Admin access required');
        const { id } = req.params;
        ...
        if (!['APPROVED', 'REJECTED'].includes(status))
            return apiResponse_1.ApiResponse.error(res, 'BAD_REQUEST', 'Status must be APPROVED or REJECTED', 400);
...
exports.adminReviewCommunityRequest = adminReviewCommunityRequest;
// ─── POLLS ───────────────────────────────────────────────────────────────────
