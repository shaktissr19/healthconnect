...
Missing core data => `INCOMPLETE_ASSESSMENT`; no numeric score is shown. Missing data never earns healthy points.
...
Up to seven readings in the last 30 days are averaged. One reading is provisional (low confidence); repeated readings on different days increase confidence.
...
Waist circumference is an optional central-adiposity risk marker using South-Asian sex-specific context. Missing waist does not reduce the Health Score; it reduces assessment depth/confidence.
...
The patient UI must include Indian tobacco exposure (cigarette, bidi, hookah and smokeless products such as gutkha, khaini, zarda). Product subtype can be expanded later; any current tobacco exposure is currently treated as current use.

### Physical activity (35%; core)
Equivalent minutes = moderate + 2 × vigorous. Scoring is progressive from inactivity through partial activity to WHO target levels and beyond; it is not all-or-none.
...
## Sleep & Recovery
Average nightly sleep is a core input and uses a progressive curve centered around the commonly recommended adult range. Both short and very long sleep reduce the component; individual clinical circumstances may differ.
...
- confidence is capped when an important recorded condition cannot yet be disease-specifically scored.
...
## Confidence
Confidence is separate from Health Score and incorporates:
- core-assessment completeness
- measurement freshness
- repeated BP evidence
- domain-specific measurement confidence
- conditional screening gaps
- unsupported disease-control limitations

A conditional core screening gap caps confidence at 75%. Unsupported disease-specific control caps confidence at 80% until better structured control information is available.
