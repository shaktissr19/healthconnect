...
const ROLES: RoleConfig[] = [
  ...
  },
  {
    ...
    headline: 'Stay supported after the consultation ends.',
    body: 'Join condition-focused communities, learn from peer experience, attend Q&A events and post anonymously where a community allows it.',
    accent: '#34D399',
  },
  {
    id: 'doctor',
  ...
  },
  {
    ...
    label: 'Knowledge Hub',
    icon: '📚',
    eyebrow: 'HEALTH KNOWLEDGE, IN FAMILIAR LANGUAGE',
...
function KnowledgePreview(){
  return <div className="lh-preview-stack">
    ...
    <div className="lh-preview-grid"><div className="lh-card"><small>POPULAR TOPICS</small><b>Heart health · diabetes · women’s health</b></div><div className="lh-card"><small>LEARN & PREPARE</small><b>Guides designed for everyday health questions</b></div></div>
  </div>;
}
...
export default function LandingHero(){
  ...
  return <section className="lh-shell">
    <style>{`
      .lh-shell{background:#fff;padding:82px 28px 18px;font-family:'DM Sans',Arial,sans-serif}.lh-wrap{max-width:1280px;height:500px;margin:0 auto;border-radius:22px;overflow:hidden;background:radial-gradient(circle at 88% 8%,rgba(20,184,166,.16),transparent 30%),linear-gradient(135deg,#061225 0%,#0A1A33 58%,#0B2943 100%);box-shadow:0 16px 42px rgba(15,23,42,.14);border:1px solid rgba(148,163,184,.15);display:grid;grid-template-rows:62px 1fr 44px}.lh-tabs{display:flex;align-items:end;gap:7px;padding:13px 28px 0;overflow-x:auto;scrollbar-width:none}.lh-tab{position:relative;overflow:hidden;border:1px solid rgba(148,163,184,.18);background:rgba(255,255,255,.035);color:#AFC2D8;border-radius:10px;padding:10px 14px;font-weight:850;font-size:11.5px;cursor:pointer;white-space:nowrap;min-height:42px}.lh-tab.active{color:#fff;border-color:var(--accent);background:rgba(255,255,255,.075)}.lh-tab.active:after{content:'';position:absolute;left:0;bottom:0;height:2px;background:var(--accent);animation:lhProgress 5.8s linear forwards}.lh-tab span{margin-right:6px}@keyframes lhProgress{from{width:0}to{width:100%}}
      .lh-main{display:grid;grid-template-columns:minmax(0,1.03fr) minmax(390px,.97fr);gap:38px;align-items:center;padding:20px 54px;height:100%;min-height:0}.lh-copy{align-self:center}.lh-eyebrow{font-size:10px;letter-spacing:.18em;font-weight:900;color:var(--accent);margin-bottom:9px}.lh-copy h1{font-family:'Sora','DM Sans',sans-serif;color:#F8FBFF;font-size:clamp(2.2rem,3.2vw,3.3rem);line-height:1.03;letter-spacing:-.045em;margin:0 0 11px;max-width:700px}.lh-copy>p{font-size:13.5px;line-height:1.58;color:#BCD0E5;margin:0 0 15px;max-width:650px}.lh-actions{display:flex;gap:9px;flex-wrap:wrap}.lh-btn{border-radius:9px;padding:10px 16px;font-size:11.5px;font-weight:900;cursor:pointer;font-family:inherit}.lh-primary{border:1px solid var(--accent);background:var(--accent);color:#061225}.lh-secondary{border:1px solid rgba(255,255,255,.24);background:rgba(255,255,255,.04);color:#fff}.lh-links{display:flex;gap:16px;flex-wrap:wrap;margin-top:10px}.lh-link{border:0;background:none;color:#91B5D7;padding:0;font-size:10.5px;font-weight:800;cursor:pointer}.lh-link:hover{color:#fff;text-decoration:underline}
      .lh-preview{background:rgba(255,255,255,.055);border:1px solid rgba(148,163,184,.18);border-radius:17px;padding:13px;box-shadow:0 18px 50px rgba(0,0,0,.2);height:235px;display:flex;flex-direction:column;justify-content:center}.lh-preview-head{display:flex;justify-content:space-between;align-items:center;color:#8CA6C4;font-size:8.5px;font-weight:900;letter-spacing:.13em;margin-bottom:9px}.lh-live{color:#86EFAC}.lh-preview-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.lh-preview-stack{display:grid;gap:8px}.lh-card{background:#F8FBFF;border:1px solid #DCE7F0;border-radius:11px;padding:10px;color:#10233C;min-width:0}.lh-card.wide{grid-column:1/-1}.lh-card.green{background:#F0FDF4;border-color:#BBF7D0}.lh-card.knowledge-feature{background:linear-gradient(135deg,#EFF6FF,#F8FBFF);border-color:#BFDBFE}.lh-card small{display:block;color:#718BA9;font-size:7.5px;font-weight:900;letter-spacing:.1em;margin-bottom:5px}.lh-card b{display:block;font-size:10.5px;line-height:1.4}.lh-card p{font-size:8.5px;line-height:1.42;color:#64748B;margin:4px 0 0}.lh-score{font-family:'Sora',sans-serif;font-size:28px;color:#0D9488;font-weight:850}.lh-score span{font-size:8px;color:#64748B}.lh-flow{display:flex;align-items:center;gap:5px;flex-wrap:wrap}.lh-flow span{font-size:7.5px;font-weight:850;background:#EDF4FA;color:#24405F;padding:4px 6px;border-radius:6px}.lh-flow i{height:1px;width:8px;background:#B8C8D9}.lh-row{display:grid;grid-template-columns:48px 1fr auto;gap:7px;align-items:center;padding:5px 0;border-bottom:1px solid #E5EDF4;font-size:8px}.lh-row:last-child{border-bottom:0}.lh-row em{font-style:normal;background:#CCFBF1;color:#0F766E;padding:3px 5px;border-radius:999px;font-size:7px}.lh-kpi{font-family:'Sora',sans-serif;font-size:24px;color:#0F4C81;font-weight:850}
      ...
      @media(max-width:930px){.lh-shell{padding-top:78px}.lh-wrap{height:auto;min-height:0;grid-template-rows:auto auto 44px}.lh-main{grid-template-columns:1fr;height:auto;padding:24px 28px 28px}.lh-preview{max-width:720px;height:auto;min-height:210px}.lh-copy h1{font-size:2.45rem}}
    ...
    <div className="lh-wrap" style={{'--accent':current.accent} as CSSProperties}>
      ...
      <div className="lh-main">
        <div className="lh-copy">
          ...
          {active==='knowledge'&&<div className="lh-actions"><button className="lh-btn lh-primary" onClick={()=>router.push('/learn')}>Explore Knowledge Hub →</button><button className="lh-btn lh-secondary" onClick={()=>signup('PATIENT')}>Create Free Account</button></div>}
        </div>
