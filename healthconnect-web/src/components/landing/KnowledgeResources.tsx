...
export default function KnowledgeResources(){
  ...
  `}</style><div className="knowledge-inner"><div className="knowledge-head"><div><div className="knowledge-kicker">KNOWLEDGE HUB</div><h2 className="knowledge-title" id="knowledge-title">Understand your health. Make better decisions.</h2></div><div className="knowledge-head-right"><p>India-focused explainers help people understand common health topics and prepare better questions for a healthcare professional.</p><div className="knowledge-head-actions"><Link href="/learn" className="knowledge-all">Visit Knowledge Hub</Link><button className="knowledge-arrow" type="button" aria-label="Previous knowledge articles" onClick={previous}>‹</button><button className="knowledge-arrow" type="button" aria-label="Next knowledge articles" onClick={next}>›</button></div></div></div><div className="knowledge-grid" aria-live="polite">{visible.map(article=><Link href={article.href} className="knowledge-card" key={`${start}-${article.href}`} style={{'--card-tint':article.tint,'--card-border':article.border,'--card-accent':article.color} as CSSProperties}>{article.visual==='cardio'?<CardioVisual/>:<div className="knowledge-photo" style={{backgroundImage:`url(${article.photo})`}} role="img" aria-label={`${article.cat} health article`}/>}<div className="knowledge-body"><span className="knowledge-cat" style={{color:article.color}}>{article.cat}</span><h3>{article.title}</h3><p className="knowledge-summary">{article.summary}</p><span className="knowledge-read">Read article</span></div></Link>)}</div><div className="knowledge-disclaimer"><strong>Educational content supports informed conversations; it is not a diagnosis or personal treatment advice.</strong></div></div></section>;
}
