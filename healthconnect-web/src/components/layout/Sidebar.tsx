...
function SvgCare(){return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 21h18"/><path d="M5 21V7l7-4 7 4v14"/><path d="M9 10h6"/><path d="M12 7v6"/><path d="M9 21v-5h6v5"/></svg>}
...
function SOSModal({ onClose, patientName }: { onClose: () => void; patientName: string }) {
  ...
  const handleSOS=async()=>{
    setSending(true);
    try{await (patientAPI as any).triggerSOS?.();}catch{}
    setSending(false);setSent(true);
  };

  return <div style={{position:'fixed',inset:0,zIndex:9999,background:'rgba(2,6,23,.76)',backdropFilter:'blur(8px)',display:'flex',alignItems:'center',justifyContent:'center',padding:16}} onClick={e=>{if(e.target===e.currentTarget)onClose()}}>
    <div style={{background:'#0F1A19',borderRadius:20,padding:'32px 28px',width:'100%',maxWidth:420,border:'1px solid rgba(220,38,38,.3)',boxShadow:'0 0 60px rgba(220,38,38,.2)'}}>
      {sent?<>
        <div style={{textAlign:'center',marginBottom:24}}><div style={{width:64,height:64,borderRadius:'50%',background:'rgba(34,197,94,.15)',border:'2px solid #22C55E',display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 16px',fontSize:28}}>✓</div><h2 style={{color:'#fff',fontSize:20,fontWeight:800,margin:'0 0 8px'}}>SOS Alert Sent</h2><p style={{color:'rgba(255,255,255,.68)',fontSize:14,margin:0}}>Your emergency contacts and nearby services have been notified.</p></div>
        ...
        <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:20}}><div style={{width:48,height:48,borderRadius:'50%',background:'rgba(220,38,38,.15)',border:'2px solid #DC2626',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,fontSize:22}}>🚨</div><div><h2 style={{color:'#fff',fontSize:18,fontWeight:800,margin:'0 0 4px'}}>Emergency SOS</h2><p style={{color:'rgba(255,255,255,.6)',fontSize:12,margin:0}}>Alert your emergency contacts and connected emergency services.</p></div><button onClick={onClose} aria-label="Close emergency dialog" style={{marginLeft:'auto',background:'none',border:'none',color:'rgba(255,255,255,.55)',cursor:'pointer',fontSize:20}}>✕</button></div>
        <div style={{background:'rgba(220,38,38,.08)',border:'1px solid rgba(220,38,38,.2)',borderRadius:12,padding:'14px 16px',marginBottom:20}}><div style={{color:'#FCA5A5',fontSize:13,fontWeight:700,marginBottom:6}}>⚠ Emergency action</div><ul style={{color:'rgba(255,255,255,.7)',fontSize:13,margin:0,paddingLeft:18,lineHeight:1.8}}><li>Your registered emergency contacts are notified</li><li>Relevant health information can be shared with connected responders</li><li>Use local emergency services immediately for a life-threatening emergency</li></ul></div>
        <div style={{color:'rgba(255,255,255,.58)',fontSize:12,textAlign:'center',marginBottom:16}}>Sending as <strong style={{color:'#fff'}}>{patientName}</strong></div>
        <div style={{display:'flex',gap:10}}><button onClick={onClose} style={{flex:1,padding:'12px 0',borderRadius:10,border:'1px solid rgba(255,255,255,.16)',background:'transparent',color:'rgba(255,255,255,.72)',fontSize:14,fontWeight:600,cursor:'pointer'}}>Cancel</button><button onClick={handleSOS} disabled={sending} style={{flex:2,padding:'12px 0',borderRadius:10,border:'none',background:sending?'rgba(220,38,38,.4)':'linear-gradient(135deg,#7F1D1D,#DC2626)',color:'#fff',fontSize:14,fontWeight:700,cursor:sending?'not-allowed':'pointer',boxShadow:'0 4px 20px rgba(220,38,38,.35)'}}>{sending?'⟳ Sending...':'🚨 Send Emergency Alert'}</button></div>
      </>}
    </div>
  </div>;
}
...
export default function Sidebar(){
  ...
  useEffect(()=>{
    ...
    Promise.allSettled([
      ...
      if(profRes.status==='fulfilled')setProfile((profRes.value as any)?.data?.data??(profRes.value as any)?.data??{});
      if(histRes.status==='fulfilled'){
        ...
        setKpis((prev:any)=>({...prev,lifeThreateningAllergy:lifeThreateningAllergy?.allergen??null}));
      }
      if(notifRes.status==='fulfilled'){
        ...
        const pendingConsents=(Array.isArray(arr)?arr:[]).filter((n:any)=>n.type==='SYSTEM'&&(n.data as any)?.requestType==='DOCTOR_ACCESS_REQUEST'&&!n.isRead).length;
  ...
  const handleNav=useCallback((id:string)=>{
    if(id==='reports'){setActivePage('my-health');setActiveTab('vault');return;}
    setActivePage(id);
  ...
  return <>
    {showSOS&&<SOSModal onClose={()=>setShowSOS(false)} patientName={displayName??'Patient'}/>} 
    <aside className="hc-sb" aria-label="My HealthConnect navigation">
      <div className="hc-sb-logo"><div className="hc-sb-logo-icon">HC</div>{!collapsed&&<div><div className="hc-sb-logo-name">HealthConnect</div><div className="hc-sb-logo-sub">PATIENT PORTAL</div></div>}</div>
      ...
      <div className="hc-sb-id">
        ...
        <div style={{display:'flex',alignItems:'center',gap:collapsed?0:8,width:'100%',justifyContent:collapsed?'center':'flex-start'}}>
          ...
          {!collapsed&&profileScore<100&&<ProfileCompletenessRing score={profileScore} size={36} onClick={()=>setActivePage('profile')}/>} 
        </div>
        {loading&&!collapsed?<><div className="hc-sb-skel" style={{height:14,width:'65%'}}/><div className="hc-sb-skel" style={{height:11,width:'42%'}}/><div className="hc-sb-skel" style={{height:11,width:'55%'}}/></>:<>
          <div className="hc-sb-id-name">{displayName??'Patient'}</div>
          <div className="hc-sb-id-reg">{regId}</div>
          {!collapsed&&<>
            <div style={{display:'flex',gap:6,flexWrap:'wrap',marginBottom:6}}>
              {profile?.dateOfBirth&&(()=>{const age=Math.floor((Date.now()-new Date(profile.dateOfBirth).getTime())/(365.25*24*3600*1000));return age>0?<span className="hc-sb-chip">{age} yrs</span>:null})()}
              {(profile?.gender??user?.gender)&&<span className="hc-sb-chip">{(profile?.gender??user?.gender)?.charAt(0).toUpperCase()+(profile?.gender??user?.gender)?.slice(1).toLowerCase()}</span>}
              {bloodDisp&&<span className="hc-sb-chip">{bloodDisp} Blood</span>}
              {isPremium&&<span className="hc-sb-chip gold">⭐ Premium</span>}
            </div>
            {kpis?.lifeThreateningAllergy&&<div style={{marginBottom:7,padding:'6px 9px',background:'#FEF2F2',border:'1px solid #FECACA',borderRadius:8,fontSize:10,fontWeight:750,color:'#B42318',display:'flex',alignItems:'center',gap:5,width:'100%'}}><span>⚠️</span> Allergy: {kpis.lifeThreateningAllergy}</div>}
            {kpis?.nextAppointmentDate&&<div style={{fontSize:10.5,color:'#53657B',display:'flex',alignItems:'center',gap:4,marginBottom:7,width:'100%',flexWrap:'wrap'}}><span>📅</span><span>Next: {new Date(kpis.nextAppointmentDate).toLocaleDateString('en-IN',{day:'numeric',month:'short'})}</span>{kpis.nextAppointmentDoctor&&<span style={{color:'#61758D'}}>· {kpis.nextAppointmentDoctor}</span>}</div>}
            {kpis?.healthScore!=null&&<div style={{width:'100%',marginTop:4}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:4}}><span style={{fontSize:9.5,color:'#52677F',textTransform:'uppercase',letterSpacing:'.08em',fontWeight:800}}>Health Score</span><span style={{fontSize:12,fontWeight:850,color:kpis.healthScore>=80?'#15803D':kpis.healthScore>=60?'#2563EB':'#B45309'}}>{kpis.healthScore}</span></div><div style={{height:5,background:'#DDE5EE',borderRadius:3,overflow:'hidden'}}><div style={{height:'100%',width:`${kpis.healthScore}%`,background:kpis.healthScore>=80?'#16A34A':kpis.healthScore>=60?'#2563EB':'#F59E0B',borderRadius:3,transition:'width .8s ease'}}/></div></div>}
          </>}
        </>}
      </div>

      {NAV.map(section=><div key={section.key}><div className="hc-sb-sec-lbl">{section.label}</div><ul className="hc-sb-nav">{section.items.map(item=>{const active=activePage===item.id;const hasBadge=item.badge!=null&&Number(item.badge)>0;const bStyle=BADGE_STYLES[item.badgeVariant??''];const IconComp=ICON_MAP[item.id]??SvgHealth;return <li key={item.id}><button className={`hc-sb-btn${active?' active':''}`} onClick={()=>handleNav(item.id)} title={collapsed?item.label:undefined} aria-current={active?'page':undefined}><span className="hc-sb-btn-icon"><IconComp/></span><span className="hc-sb-btn-lbl">{item.label}</span>{hasBadge&&<><span className="hc-sb-badge" style={bStyle}>{item.badge}</span><span className="hc-sb-dot" style={{background:bStyle.background}}/></>}</button></li>})}</ul><div className="hc-sb-divider"/></div>)}

      <div className="hc-sb-bottom"><button className="hc-sos-btn" onClick={()=>setShowSOS(true)}><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>{!collapsed&&'Emergency SOS'}</button><button className="hc-logout-btn" onClick={handleLogout}><SvgLogout/>{!collapsed&&'Log Out'}</button></div>
    </aside>
  </>;
}
