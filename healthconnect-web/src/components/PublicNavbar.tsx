...
function UserMenu({user,onSignOut}:{user:any;onSignOut:()=>void}){
  ...
  return <div ref={ref} style={{position:'relative'}}>
    <button onClick={()=>setOpen(v=>!v)} className="hc-user-trigger"><span className="hc-user-avatar">{initials}</span><span className="hc-user-name">{user?.firstName||'Account'}</span><span style={{fontSize:9,color:'#A7D7D0'}}>⌄</span></button>
...
export default function PublicNavbar(){
  ...
  return <>
    <style>{`
      ...
      .hc-hamburger{display:none;margin-left:auto;border:1px solid rgba(255,255,255,.45);background:rgba(255,255,255,.08);border-radius:9px;width:40px;height:40px;cursor:pointer;font-size:18px;color:#fff}.hc-mobile-menu{position:fixed;z-index:999;top:74px;left:0;right:0;bottom:0;background:#fff;padding:17px;transform:translateX(100%);transition:.25s;overflow-y:auto;font-family:'DM Sans',Arial,sans-serif}.hc-mobile-menu.open{transform:translateX(0)}.hc-mobile-menu button{width:100%;border:1px solid #E6EEF1;background:#F8FBFC;border-radius:10px;padding:12px 13px;text-align:left;color:#17324D;font-size:14px;font-weight:700;cursor:pointer;margin-bottom:5px}.hc-mobile-menu button.active{background:#E9F8F5;border-color:#B7E4DC;color:#0F766E}.hc-mobile-divider{height:1px;background:#E6EEF1;margin:10px 0}.hc-mobile-help{font-size:9px;color:#71879A;padding:0 13px 7px;line-height:1.45}.hc-mobile-auth{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:12px}.hc-mobile-auth button{text-align:center;margin:0}.hc-mobile-auth .primary{background:#075B57;color:#fff;border-color:#075B57}
    ...
    `}</style>
    <nav className={`hc-public-nav ${scrolled?'scrolled':''}`}><div className="hc-progress" style={{width:`${scrollPct}%`}}/><div className="hc-nav-inner"><button className="hc-brand" onClick={goHome}><span className="hc-brand-logo">HC</span><span className="hc-brand-copy"><strong>HealthConnect</strong><span>India&apos;s Unified Healthcare Platform</span></span></button><div className="hc-nav-links">{links.map(([label,href])=><button key={href} className={`hc-nav-link ${active(href)?'active':''}`} onClick={()=>go(href)}>{label}</button>)}<button className="hc-nav-link" onClick={goPlans}>Plans</button><div className="hc-nav-separator"/><button className="hc-nav-link" title="Your reports, medicines, appointments and health journey" onClick={()=>accountShortcut('health')}>My Health</button><button className="hc-nav-link" title="Doctor workspace for patients, schedules and consultations" onClick={()=>accountShortcut('patients')}>My Patients</button></div><div className="hc-nav-auth">{_hasHydrated&&isAuthenticated&&user?<><button className="hc-dashboard-btn" onClick={()=>router.push(getDashRoute(user.role))}>My Dashboard</button><UserMenu user={user} onSignOut={clearAuth}/></>:<><button className="hc-signin" onClick={()=>requestAuth('login')}>Sign In</button><button className="hc-signup" onClick={()=>requestAuth('register')}>Sign Up</button></>}</div><button className="hc-hamburger" aria-label="Open menu" onClick={()=>setMobileOpen(v=>!v)}>{mobileOpen?'×':'☰'}</button></div></nav>
    <div className={`hc-mobile-menu ${mobileOpen?'open':''}`}>{links.map(([label,href])=><button key={href} className={active(href)?'active':''} onClick={()=>go(href)}>{label} <span style={{float:'right',opacity:.4}}>→</span></button>)}<button onClick={goPlans}>Plans <span style={{float:'right',opacity:.4}}>→</span></button><div className="hc-mobile-divider"/><button onClick={()=>accountShortcut('health')}>♡ My Health <span style={{float:'right',opacity:.4}}>→</span></button><div className="hc-mobile-help">Your reports, medicines, appointments and health journey.</div><button onClick={()=>accountShortcut('patients')}>👥 My Patients <span style={{float:'right',opacity:.4}}>→</span></button><div className="hc-mobile-help">Doctor workspace for patients, schedules and consultations.</div><div className="hc-mobile-auth">{isAuthenticated&&user?<><button className="primary" onClick={()=>{router.push(getDashRoute(user.role));setMobileOpen(false)}}>My Dashboard</button><button onClick={()=>{clearAuth();setMobileOpen(false);goHome()}}>Sign Out</button></>:<><button onClick={()=>requestAuth('login')}>Sign In</button><button className="primary" onClick={()=>requestAuth('register')}>Sign Up</button></>}</div></div>
  </>;
}
