...
function UserMenu({user,onSignOut}:{user:any;onSignOut:()=>void}){
  ...
  return <div ref={ref} style={{position:'relative'}}>
    <button onClick={()=>setOpen(v=>!v)} className="hc-user-trigger"><span className="hc-user-avatar">{initials}</span><span className="hc-user-name">{user?.firstName||'Account'}</span><span style={{fontSize:9,color:'#C9DBE7'}}>⌄</span></button>
...
export default function PublicNavbar(){
  ...
  return <>
    <style>{`
      .hc-public-nav{position:fixed;top:0;left:0;right:0;height:64px;z-index:1000;background:linear-gradient(92deg,#07353B 0%,#0A3B46 48%,#0B3150 100%);border-bottom:1px solid rgba(94,234,212,.38);font-family:'DM Sans',Arial,sans-serif;transition:.2s;box-shadow:0 8px 24px rgba(6,30,42,.16)}
      .hc-public-nav.scrolled{background:linear-gradient(92deg,#062E35 0%,#083743 48%,#092D49 100%);box-shadow:0 10px 30px rgba(6,30,42,.22)}
      .hc-nav-inner{max-width:1280px;height:100%;margin:0 auto;padding:0 28px;display:flex;align-items:center}
      .hc-brand{display:flex;align-items:center;gap:10px;border:0;background:none;padding:0;margin-right:18px;cursor:pointer;flex-shrink:0}
      .hc-brand-logo{width:40px;height:40px;border-radius:11px;background:linear-gradient(135deg,#14B8A6,#2DD4BF);display:grid;place-items:center;color:#fff;font-family:'Sora',sans-serif;font-weight:900;box-shadow:0 5px 18px rgba(45,212,191,.24)}
      .hc-brand-copy{text-align:left}.hc-brand-copy strong{display:block;font-family:'Sora',sans-serif;color:#F8FBFF;font-size:14px;line-height:1.05}.hc-brand-copy span{display:block;color:#A7F3D0;font-size:9px;font-weight:700;margin-top:3px;white-space:nowrap}
      .hc-nav-links{display:flex;align-items:center;gap:1px;flex:1}.hc-nav-link{border:1px solid transparent;background:transparent;border-radius:8px;padding:7px 9px;color:#DCEAF2;font-size:12.5px;font-weight:650;white-space:nowrap;cursor:pointer;transition:.16s}
      .hc-nav-link:hover{color:#fff;background:rgba(255,255,255,.08);border-color:rgba(167,243,208,.22)}.hc-nav-link.active{color:#fff;background:rgba(45,212,191,.16);border-color:rgba(94,234,212,.5);box-shadow:0 3px 12px rgba(20,184,166,.12)}
      .hc-nav-separator{width:1px;height:22px;background:rgba(207,224,227,.3);margin:0 5px}.hc-nav-auth{display:flex;gap:8px;align-items:center;margin-left:10px}
      .hc-signin,.hc-signup,.hc-dashboard-btn{border-radius:9px;padding:8px 15px;font-size:12px;font-weight:800;cursor:pointer;white-space:nowrap}.hc-signin{border:1px solid rgba(255,255,255,.48);background:rgba(255,255,255,.92);color:#17354B}.hc-signup{border:1px solid #2DD4BF;background:linear-gradient(135deg,#14B8A6,#2DD4BF);color:#062F34;box-shadow:0 5px 16px rgba(20,184,166,.24)}.hc-dashboard-btn{border:1px solid rgba(255,255,255,.38);background:rgba(255,255,255,.1);color:#fff}
      .hc-user-trigger{display:flex;align-items:center;gap:7px;border:1px solid rgba(94,234,212,.35);background:rgba(255,255,255,.09);border-radius:9px;padding:4px 9px 4px 4px;cursor:pointer}.hc-user-avatar{width:29px;height:29px;border-radius:50%;display:grid;place-items:center;background:linear-gradient(135deg,#14B8A6,#2DD4BF);color:#fff;font-size:10px;font-weight:900}.hc-user-avatar.big{width:42px;height:42px;font-size:14px;margin:0 auto 7px}.hc-user-name{font-size:11px;font-weight:800;color:#F8FBFF;max-width:72px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
      .hc-user-menu{position:absolute;right:0;top:calc(100% + 8px);width:220px;background:#fff;border:1px solid #E2E8F0;border-radius:14px;box-shadow:0 14px 40px rgba(15,23,42,.14);overflow:hidden}.hc-user-head{text-align:center;padding:15px;border-bottom:1px solid #EEF2F6}.hc-user-head strong{display:block;font-size:12px;color:#0F172A}.hc-user-head span{display:block;font-size:9px;color:#94A3B8;margin-top:2px;overflow:hidden;text-overflow:ellipsis}.hc-user-menu>button{width:100%;display:flex;justify-content:space-between;border:0;border-bottom:1px solid #F1F5F9;background:#fff;padding:10px 13px;color:#475569;font-size:11px;font-weight:650;cursor:pointer;text-align:left}.hc-user-menu>button:hover{background:#F8FAFC}.hc-user-menu>button.danger{color:#DC2626}
      .hc-progress{position:absolute;bottom:0;left:0;height:2px;background:linear-gradient(90deg,#2DD4BF,#67E8F9,#60A5FA);box-shadow:0 0 9px rgba(45,212,191,.48)}
      .hc-hamburger{display:none;margin-left:auto;border:1px solid rgba(255,255,255,.35);background:rgba(255,255,255,.1);border-radius:8px;width:38px;height:38px;cursor:pointer;font-size:18px;color:#fff}
      .hc-mobile-menu{position:fixed;z-index:999;top:64px;left:0;right:0;bottom:0;background:linear-gradient(160deg,#062E35,#0B3150);padding:17px;transform:translateX(100%);transition:.25s;overflow-y:auto;font-family:'DM Sans',Arial,sans-serif}.hc-mobile-menu.open{transform:translateX(0)}.hc-mobile-menu button{width:100%;border:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.06);border-radius:10px;padding:12px 13px;text-align:left;color:#DCEAF2;font-size:14px;font-weight:700;cursor:pointer;margin-bottom:3px}.hc-mobile-menu button.active{background:rgba(45,212,191,.14);border-color:rgba(94,234,212,.4);color:#fff}.hc-mobile-divider{height:1px;background:rgba(214,231,231,.18);margin:9px 0}.hc-mobile-help{font-size:9px;color:#9CB2C4;padding:0 13px 7px;line-height:1.45}.hc-mobile-auth{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:12px}.hc-mobile-auth button{text-align:center;margin:0}.hc-mobile-auth .primary{background:#2DD4BF;color:#062F34;border-color:#2DD4BF}
