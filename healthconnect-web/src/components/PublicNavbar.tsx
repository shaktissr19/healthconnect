...
export default function PublicNavbar(){
  ...
  const accountShortcut=(intent:'health'|'patients')=>{
    setMobileOpen(false);
    if(!isAuthenticated||!user){try{sessionStorage.setItem('hc_post_login_redirect',intent==='health'?'/dashboard':'/doctor-dashboard?tab=patients')}catch{}requestAuth('login');return;}
  ...
  return <>
    ...
    <div id="hc-mobile-menu" className={`hc-mobile-menu ${mobileOpen?'open':''}`} aria-hidden={!mobileOpen}>{links.map(([label,href])=><button type="button" key={href} className={active(href)?'active':''} aria-current={active(href)?'page':undefined} onClick={()=>go(href)}>{label}</button>)}<button type="button" onClick={goPlans}>Plans</button><div className="hc-mobile-divider"/><button type="button" onClick={()=>accountShortcut('health')}>My Health</button><div className="hc-mobile-help">Your reports, medicines, appointments and health journey.</div><button type="button" onClick={()=>accountShortcut('patients')}>My Patients</button><div className="hc-mobile-help">Doctor workspace for patients, schedules and consultations.</div><div className="hc-mobile-auth">{isAuthenticated&&user?<><button type="button" className="primary" onClick={()=>{router.push(getDashRoute(user.role));setMobileOpen(false)}}>My Dashboard</button><button type="button" onClick={()=>{clearAuth();setMobileOpen(false);goHome()}}>Sign Out</button></>:<><button type="button" onClick={()=>requestAuth('login')}>Sign In</button><button type="button" className="primary" onClick={()=>requestAuth('register')}>Sign Up</button></>}</div></div>
  </>;
}
