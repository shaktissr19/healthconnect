...
function UserMenu({user,onSignOut}:{user:any;onSignOut:()=>void}){
  ...
  return <div ref={ref} style={{position:'relative'}}>
    <button onClick={()=>setOpen(v=>!v)} className="hc-user-trigger"><span className="hc-user-avatar">{initials}</span><span className="hc-user-name">{user?.firstName||'Account'}</span><span style={{fontSize:9,color:'#64748B'}}>⌄</span></button>
...
export default function PublicNavbar(){
  ...
  return <>
    <style>{`
      .hc-public-nav{position:fixed;top:0;left:0;right:0;height:74px;z-index:1000;background:rgba(255,255,255,.98);border-bottom:1px solid #E5ECEF;font-family:'DM Sans',Arial,sans-serif;transition:box-shadow .2s,background .2s;box-shadow:0 2px 10px rgba(15,49,66,.04)}
      .hc-public-nav.scrolled{background:rgba(255,255,255,.97);backdrop-filter:blur(16px);box-shadow:0 8px 28px rgba(15,49,66,.10)}
      .hc-nav-inner{max-width:1480px;height:100%;margin:0 auto;padding:0 30px;display:flex;align-items:center}
      .hc-brand{display:flex;align-items:center;gap:11px;border:0;background:none;padding:0;margin-right:34px;cursor:pointer;flex-shrink:0}
      .hc-brand-logo{width:43px;height:43px;border-radius:12px;background:linear-gradient(135deg,#0F766E,#14B8A6);display:grid;place-items:center;color:#fff;font-family:'Sora',sans-serif;font-size:14px;font-weight:900;box-shadow:0 5px 16px rgba(13,148,136,.20)}
      .hc-brand-copy{text-align:left}.hc-brand-copy strong{display:block;font-family:'Sora',sans-serif;color:#0B2540;font-size:20px;line-height:1.02;letter-spacing:-.025em}.hc-brand-copy span{display:block;color:#0F8C7F;font-size:8.5px;font-weight:750;margin-top:4px;white-space:nowrap;letter-spacing:.01em}
      .hc-nav-links{display:flex;align-items:center;gap:4px;flex:1}.hc-nav-link{position:relative;border:0;background:transparent;border-radius:0;padding:25px 9px 22px;color:#17324D;font-size:12.5px;font-weight:750;white-space:nowrap;cursor:pointer;transition:color .16s}
      .hc-nav-link:after{content:'';position:absolute;left:9px;right:9px;bottom:13px;height:2px;border-radius:2px;background:#0D9488;transform:scaleX(0);transform-origin:center;transition:transform .16s}
      .hc-nav-link:hover{color:#0D9488}.hc-nav-link:hover:after,.hc-nav-link.active:after{transform:scaleX(1)}.hc-nav-link.active{color:#0D9488}
      .hc-nav-separator{width:1px;height:24px;background:#E2E8F0;margin:0 5px}.hc-nav-auth{display:flex;gap:9px;align-items:center;margin-left:14px}
      .hc-signin,.hc-signup,.hc-dashboard-btn{border-radius:10px;padding:10px 17px;font-size:12px;font-weight:850;cursor:pointer;white-space:nowrap;transition:transform .16s,box-shadow .16s,background .16s}.hc-signin{border:1px solid #9ECFC9;background:#fff;color:#0F766E}.hc-signin:hover{background:#F0FDFA}.hc-signup{border:1px solid #0D9488;background:linear-gradient(135deg,#0F766E,#14B8A6);color:#fff;box-shadow:0 6px 17px rgba(13,148,136,.20)}.hc-signup:hover{transform:translateY(-1px);box-shadow:0 8px 22px rgba(13,148,136,.24)}.hc-dashboard-btn{border:1px solid #B9DAD6;background:#F0FDFA;color:#0F766E}.hc-dashboard-btn:hover{background:#DDF7F2}
      .hc-user-trigger{display:flex;align-items:center;gap:7px;border:1px solid #D5E6E4;background:#fff;border-radius:10px;padding:4px 9px 4px 4px;cursor:pointer;box-shadow:0 3px 10px rgba(15,49,66,.05)}.hc-user-avatar{width:30px;height:30px;border-radius:50%;display:grid;place-items:center;background:linear-gradient(135deg,#0F766E,#14B8A6);color:#fff;font-size:10px;font-weight:900}.hc-user-avatar.big{width:42px;height:42px;font-size:14px;margin:0 auto 7px}.hc-user-name{font-size:11px;font-weight:800;color:#17324D;max-width:72px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
      .hc-user-menu{position:absolute;right:0;top:calc(100% + 9px);width:220px;background:#fff;border:1px solid #E2E8F0;border-radius:14px;box-shadow:0 14px 40px rgba(15,23,42,.14);overflow:hidden}.hc-user-head{text-align:center;padding:15px;border-bottom:1px solid #EEF2F6}.hc-user-head strong{display:block;font-size:12px;color:#0F172A}.hc-user-head span{display:block;font-size:9px;color:#94A3B8;margin-top:2px;overflow:hidden;text-overflow:ellipsis}.hc-user-menu>button{width:100%;display:flex;justify-content:space-between;border:0;border-bottom:1px solid #F1F5F9;background:#fff;padding:10px 13px;color:#475569;font-size:11px;font-weight:650;cursor:pointer;text-align:left}.hc-user-menu>button:hover{background:#F8FAFC}.hc-user-menu>button.danger{color:#DC2626}
      .hc-progress{position:absolute;bottom:0;left:0;height:2px;background:linear-gradient(90deg,#0D9488,#2DD4BF,#38BDF8);box-shadow:0 0 8px rgba(13,148,136,.24)}
      .hc-hamburger{display:none;margin-left:auto;border:1px solid #CDE1DF;background:#F8FBFC;border-radius:9px;width:40px;height:40px;cursor:pointer;font-size:18px;color:#0F766E}
      .hc-mobile-menu{position:fixed;z-index:999;top:74px;left:0;right:0;bottom:0;background:#fff;padding:17px;transform:translateX(100%);transition:.25s;overflow-y:auto;font-family:'DM Sans',Arial,sans-serif}.hc-mobile-menu.open{transform:translateX(0)}.hc-mobile-menu button{width:100%;border:1px solid #E6EEF1;background:#F8FBFC;border-radius:10px;padding:12px 13px;text-align:left;color:#17324D;font-size:14px;font-weight:700;cursor:pointer;margin-bottom:5px}.hc-mobile-menu button.active{background:#E9F8F5;border-color:#B7E4DC;color:#0F766E}.hc-mobile-divider{height:1px;background:#E6EEF1;margin:10px 0}.hc-mobile-help{font-size:9px;color:#71879A;padding:0 13px 7px;line-height:1.45}.hc-mobile-auth{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:12px}.hc-mobile-auth button{text-align:center;margin:0}.hc-mobile-auth .primary{background:#0D9488;color:#fff;border-color:#0D9488}
      @media(max-width:1120px){.hc-nav-links,.hc-nav-auth{display:none}.hc-hamburger{display:block}.hc-brand{margin-right:0}}
      @media(max-width:480px){.hc-nav-inner{padding:0 14px}.hc-brand-copy span{display:none}.hc-brand-copy strong{font-size:17px}.hc-brand-logo{width:39px;height:39px}}
    `}</style>
