...
export default function SubscriptionsPage(){
  ...
  const metrics=useMemo(()=>{
    ...
    const started=paid.reduce((n:number,p:any)=>n+Number(p.totalCount??(Number(p.activeCount||0)+Number(p.trialingCount||0)+Number(p.pastDueCount||0)+Number(p.cancelledCount||0)+Number(p.expiredCount||0))),0);
  ...
  const membershipsStarted=Number(totals.membershipsStarted??metrics.started);
  const activePaid=Number(totals.activePaidSubscribers??totals.activeSubscribers??metrics.activePaid);
  const patientActive=Number(totals.patientActiveSubscribers??metrics.patientActive);
  const doctorActive=Number(totals.doctorActiveSubscribers??metrics.doctorActive);
  const mrrPaise=Number(totals.estimatedMrrPaise??metrics.mrr);
  ...
  return <div style={{color:C.text,fontFamily:"'Inter',sans-serif"}}>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:12,marginBottom:22}}>
      <Metric label="Memberships started" value={membershipsStarted} color={C.text}/>
      <Metric label="Active paid members" value={activePaid} color={C.green}/>
      <Metric label="Patient members" value={patientActive} color={C.blue}/>
      <Metric label="Doctor members" value={doctorActive} color={C.teal}/>
      <Metric label="Monthly recurring value" value={money(mrrPaise)} color={C.amber}/>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(165px,1fr))',gap:11,marginBottom:24}}>
      <Small label="Trialing" value={Number(totals.trialingSubscribers||0)} color={C.blue}/>
      ...
      <Small label="Expired" value={Number(totals.expiredSubscribers||0)} color={C.muted}/>
...
function Empty(){return <div style={{padding:36,textAlign:'center',color:C.muted,fontSize:12.5}}>No paid membership records yet.</div>}
